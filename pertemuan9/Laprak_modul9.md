...
Program di atas merupakan implementasi struktur data pohon biner dalam bahasa C++. Pohon biner adalah struktur data yang terdiri dari simpul (node) di mana setiap simpul memiliki paling banyak dua anak, yaitu anak kiri dan anak kanan. Program ini menggunakan struktur `Pohon` untuk mendefinisikan simpul dengan atribut seperti `data` untuk menyimpan karakter, serta pointer `left`, `right`, dan `parent` untuk menghubungkan simpul dengan anak-anak dan orang tua mereka. Variabel global `root` digunakan sebagai pointer ke simpul akar pohon, dan `baru` digunakan untuk membuat simpul baru.
Fungsi-fungsi utama dalam program ini mencakup `init` untuk menginisialisasi pohon, `isEmpty` untuk memeriksa apakah pohon kosong, serta `buatNode`, `insertLeft`, dan `insertRight` untuk menambahkan simpul ke pohon. Selain itu, terdapat fungsi `update` untuk memperbarui data pada simpul tertentu, `retrieve` untuk menampilkan data simpul, dan `find` untuk menampilkan informasi lengkap tentang simpul, termasuk data, parent, sibling, dan anak-anaknya. Program ini juga memiliki fungsi untuk traversal pohon seperti `preOrder`, `inOrder`, dan `postOrder`, serta fungsi `deleteTree` dan `deleteSub` untuk menghapus pohon atau subtree tertentu. Fungsi `clear` digunakan untuk menghapus seluruh pohon, sementara `size` dan `height` digunakan untuk menghitung ukuran dan tinggi pohon. Fungsi `characteristic` menampilkan karakteristik pohon termasuk ukuran, tinggi, dan rata-rata node. Untuk mencari simpul dalam pohon berdasarkan nilai, digunakan fungsi `findNode`. Program ini juga menyediakan menu interaksi pengguna melalui fungsi `displayMenu` yang memungkinkan pengguna memilih berbagai operasi pada pohon, seperti membuat tree, menambah anak kiri atau kanan, mengubah data simpul, menampilkan data simpul, mencari simpul, melakukan traversal, menghapus subtree atau seluruh pohon, serta menampilkan karakteristik pohon. Fungsi `main` mengelola interaksi pengguna dengan menampilkan menu, menerima input, dan memanggil fungsi terkait berdasarkan pilihan pengguna. Dengan demikian, program ini memungkinkan pengguna untuk membuat, memanipulasi, dan menampilkan informasi serta karakteristik dari pohon biner dengan mudah.

## Kesimpulan
Program graph dan tree merupakan implementasi dasar dari dua struktur data yang penting dalam pemrograman, yaitu graph dan tree. Program graph menggunakan adjacency list untuk merepresentasikan kumpulan simpul yang saling terhubung oleh edge. Dalam program ini, kita dapat membuat simpul baru, menambahkan edge antara simpul, serta melakukan traversal graph. Program tree menggunakan pointer untuk merepresentasikan hierarki kumpulan node, dengan satu node sebagai root. Program ini memungkinkan kita untuk membuat node, menambahkan child kiri dan kanan, mengubah data node, serta melakukan operasi lain seperti mencari informasi tentang parent, sibling, dan child. Selain itu, program tree juga menyediakan fungsi-fungsi untuk melakukan traversal tree seperti pre-order, in-order, dan post-order. Kedua program ini memberikan pemahaman tentang struktur data yang digunakan untuk mengorganisasi dan memanipulasi data yang memiliki hubungan kompleks dan hirarkis.<br/>
Baik graph maupun tree adalah struktur data yang sangat berguna dan efisien dalam berbagai konteks aplikasi. Graph lebih cocok untuk data dengan banyak relasi kompleks, sementara tree lebih sesuai untuk data yang terorganisir secara hierarkis. Kedua struktur data ini memungkinkan representasi yang jelas dan berbagai operasi manipulatif yang diperlukan untuk mengelola data secara efektif. Dengan demikian, penggunaan graph dan tree sebagai struktur data dalam program-program di atas menunjukkan kemampuan mereka dalam mengelola, merepresentasikan, dan memanipulasi data dengan cara yang efisien dan terstruktur.<br/>
